...
export async function mountSeoWorkspace({supabase,company}) {
  ...
  function google() {
    ...
    ${!g.configured?`<div class="seo-notice">Google connection setup is pending.${g.setupIssues?.length?`<p>Ask the administrator to correct these settings for this deployment:</p><ul>${g.setupIssues.map(issue=>`<li>${esc(issue)}</li>`).join('')}</ul>`:' The administrator must complete the Google configuration before accounts can connect.'}</div>`:`<div class="seo-actions"><button class="seo-primary" data-action="google-start">${g.connected?'Reconnect Google':'Connect Google'}</button>${g.connected?'<button data-action="google-sites">Choose / change property</button><button data-action="google-disconnect">Disconnect</button>':''}</div>`}
