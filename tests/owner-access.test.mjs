...
test('owner endpoint enforces auth before all platform queries',async()=>{
 ...
 try {
  let r=res();await handler({method:'GET',headers:{}},r);assert.equal(r.code,401);assert.equal(calls.length,0);
  r=res();await handler({method:'POST',headers:{}},r);assert.equal(r.code,405);assert.equal(calls.length,0);
  r=res();await handler({method:'GET',headers:{authorization:'Bearer bad'},query:{user_id:owner}},r);assert.equal(r.code,403);assert.equal(calls.length,1);assert.match(calls[0].url,/auth\/v1\/user$/);
  assert.equal(r.headers['Cache-Control'],'private, no-store');
  global.fetch=async()=>new Response('{}',{status:401});r=res();await handler({method:'GET',headers:{authorization:'Bearer invalid'}},r);assert.equal(r.code,401);
...
test('authorized owner data is bounded, sanitized, and reports failed totals as unavailable',async()=>{
 ...
 try{
  let r=res();await handler({method:'GET',headers:{authorization:'Bearer valid'},query:{}},r);
  ...
  r=res();await handler({method:'GET',headers:{authorization:'Bearer valid'},query:{page:'-1'}},r);assert.equal(r.code,400);
 }finally{global.fetch=oldFetch;for(const key of Object.keys(process.env))if(!(key in oldEnv))delete process.env[key];Object.assign(process.env,oldEnv)}
});
