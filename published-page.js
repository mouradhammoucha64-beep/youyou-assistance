...
function hardenPublishedHtml(html = "") {
  ...
  if (!/name=["']color-scheme["']/i.test(out)) {
    out = out.replace(/<head([^>]*)>/i, '<head$1><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light">');
  }
  if (!/youyou-runtime-color-lock/i.test(out)) {
    out = out.replace(/<\/head>/i, '<style id="youyou-runtime-color-lock">html{color-scheme:only light!important}body{-webkit-text-size-adjust:100%;text-size-adjust:100%}@media(max-width:760px){.lp-image-slide,.beauty-wow .lp-image-slide{flex-basis:82%!important;width:82%!important;min-width:82%!important}}</style></head>');
  }
  return out;
}
